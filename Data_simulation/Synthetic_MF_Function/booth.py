...
class booth():
    def __init__(self, cost_type, total_fidelity_num = None):
        self.x_dim = 2
        self.search_range = [[-10, 10], [-10, 10], [0, 1]]
    ...
    def _booth_hf(self, xx):
        
        x1, x2 = xx[:,0], xx[:,1]
        return (x1 + 2*x2 - 7)**2 + (2*x1 + x2 - 5)**2
    
    def booth_lf(self, xx):

        x1, x2 = xx[:,0], xx[:,1]
        
        term1 = self._booth_hf(torch.hstack([.4*x1.reshape(-1,1), x2.reshape(-1,1)]))
    ...
    def get_cmf_data(self, input_x, input_s):
        xtr = input_x
        y_l = self.booth_lf(xtr).reshape(-1,1)
        y_h = self._booth_hf(xtr).reshape(-1,1)
