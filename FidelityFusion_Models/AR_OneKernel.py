...
'''
the method maybe have some bugs, need to be checked
'''
...
class AR_union(nn.Module):
    ...
    def __init__ (self, fidelity_num, kernel_list, rho_init = 1.0):
        """
        Initializes an instance of the AR_OneKernel class.

        Parameters:
        - fidelity_num (int): The number of fidelity levels.
        - kernel_list (list): A list of kernel modules.
        - rho_init (float): The initial value for rho.
    ...
    def forward(self, data_manager, x_test, fidelity_indicator = None, normal = False):
        ...
        """
        Forward pass of the AR_OneKernel model.
        Args:
    ...
    def negative_log_likelihood(self, x_train, y_train):
        ...
        Parameters:
        - x_train (list): A list of input training data arrays.
        - y_train (array): The target training data array.
        Returns:
    ...
    def cal_Union_sigma(self, x_num, x_train):
        ...
        Args:
            x_num (int): The number of elements in x_train.
    ...
    def cal_union_kx(self, xtr_num, xte_num, x_train, x_test, to_fidelity):
        ...
        Args:
            x1_num (int): Number of train data.
            x2_num (int): Number of test data.
            x_train (list): List of input training data.
            x_test (array-like): Input test data.
            to_fidelity (int): Index of the fidelity level.

        Returns:
            torch.Tensor: The union of kernel matrices.

        Raises:
            IndexError: If `to_fidelity` is out of range.
        ...
        for f_r in range(self.fidelity_num): ##只有一竖向的分块矩阵
            ...
            if f_r == to_fidelity:
                ...
                for i in range(f_r):
                    kx[cur_row:cur_row+xf_num,0:xte_num] += self.rho_list[i]**2 * self.kernel_list[i+1](x_f, x_test)
            elif f_r < to_fidelity:
                kx[cur_row:cur_row+xf_num,0:xte_num] = self.rho_list[to_fidelity-1] * self.kernel_list[to_fidelity-1](x_f, x_test)
            else:
                kx[cur_row:cur_row+xf_num,0:xte_num] = self.rho_list[rho_num] * self.kernel_list[rho_num](x_test, x_f)
                rho_num += 1
            cur_row += xf_num
        return kx   
...
def train_AR_union(GPmodel, data_manager, max_iter=100, lr_init=1e-1, normal = False):
    ...
    """
    Trains an AR (AutoRegressive) model using union form from multiple fidelity levels.
    Args:
        GPmodel (torch.nn.Module): The AR model to be trained.
        data_manager: The data manager object that provides access to the data.
        ...
        normal (bool, optional): Flag indicating whether to normalize the data. Defaults to False.
    Returns:
        None
    """
