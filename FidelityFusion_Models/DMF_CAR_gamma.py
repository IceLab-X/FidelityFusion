...
class fidelity_kernel_MCMC(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        """
        # lf, hf = warp_function(self.lf, self.hf, 2)
        # cdf_gamma_lf = gamma.cdf(lf, 1, scale=1)
        # cdf_gamma_hf = gamma.cdf(hf, 1, scale=1)

        cdf_gamma_lf = gamma.cdf(self.lf, 1, scale=1)
        cdf_gamma_hf = gamma.cdf(self.hf, 1, scale=1)

        final_part = (torch.ones(1) * (cdf_gamma_hf - cdf_gamma_lf)).exp() - 1
...
def train_DMFCAR_gamma(CARmodel, data_manager, max_iter=1000, lr_init=1e-1, normal =True, debugger = None):
...
if __name__ == "__main__":
    ...
    CAR = DMF_CAR_gamma(fidelity_num=fidelity_num, kernel_list=kernel_list, b_init=1.0).to(device)

    train_DMFCAR_gamma(CAR,fidelity_manager, max_iter=200, lr_init=1e-2, debugger = None)
