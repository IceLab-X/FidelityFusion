...
def train_GARres(GARmodel, data_manager, max_iter=1000, lr_init=1e-1, debugger=None):
    """
    ...
    for i_fidelity in range(GARmodel.fidelity_num):
        ...
        if i_fidelity == 0:
            ...
            for i in range(max_iter):
                optimizer.zero_grad()
                if GARmodel.if_nonsubset:
                    y_residual_mean = y_high[0] -  y_low[0]  # tensor linear layer
...
if __name__ == "__main__":
    ...
    myGAR = GAR_res(fidelity_num, kernel_list, data_shape, if_nonsubset = True).to(device)

    train_GARres(myGAR, fidelity_manager, max_iter = 100, lr_init = 1e-3, debugger = debugger)
