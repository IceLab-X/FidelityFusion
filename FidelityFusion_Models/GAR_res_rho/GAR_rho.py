...
class GAR_rho(torch.nn.Module):
    ...
    def __init__(self, fidelity_num, kernel_list, data_shape_list, if_nonsubset=False):
        ...
        for _ in range(self.fidelity_num - 1):
            self.rho_list.append(torch.nn.Parameter(torch.tensor(1.0)))
    ...
    def forward(self, data_manager, x_test, to_fidelity=None):
        ...
        for i_fidelity in range(fidelity_level + 1):
            if i_fidelity == 0:
                ...
                mean_high = self.rho_list[i_fidelity - 1]*mean_low + mean_res
                var_high = self.rho_list[i_fidelity - 1]*var_low + var_res
...
def train_GARrho(GARmodel, data_manager, max_iter=1000, lr_init=1e-1, debugger=None):
    """
    ...
    for i_fidelity in range(GARmodel.fidelity_num):
        ...
        if i_fidelity == 0:
            ...
            for i in range(max_iter):
                optimizer.zero_grad()
                if GARmodel.if_nonsubset:
                    y_residual_mean = y_high[0] - GARmodel.rho_list[i_fidelity - 1] * y_low[0]  # tensor linear layer
                    y_residual_var = abs(y_high[1] - y_low[1])
                else:
                    y_residual_mean = y_high - GARmodel.rho_list[i_fidelity - 1] * y_low
                    y_residual_var = None
...
if __name__ == "__main__":
    ...
    myGAR = GAR_rho(fidelity_num, kernel_list, data_shape, if_nonsubset = True).to(device)

    train_GARrho(myGAR, fidelity_manager, max_iter = 100, lr_init = 1e-3, debugger = debugger)
