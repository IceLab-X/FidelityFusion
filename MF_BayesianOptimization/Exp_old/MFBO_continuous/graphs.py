...
methods_name_list = ['FiDEs_UCB', 'FiDEs_ES', 'FiDEs_cfKG']
line = []
for methods_name in methods_name_list:
    ct = []
    tem = []
    for seed in [0]:
        path = os.path.join(sys.path[-1], 'Experiment', 'Exp_continuous_1', 'Exp_results',
...
plt.tight_layout()
plt.savefig(os.path.join(sys.path[-1], 'Experiment', 'Exp_continuous_1', 'Graphs') + '/' + data_name + '.png',
            bbox_inches='tight')
