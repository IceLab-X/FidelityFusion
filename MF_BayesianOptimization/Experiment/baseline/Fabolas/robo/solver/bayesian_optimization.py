...
class BayesianOptimization(BaseSolver):
    ...
    def run(self, num_iterations=10, X=None, y=None):
        ...
        for it in range(self.init_points, num_iterations):
            ...
            new_x = self.choose_next(self.X, self.y, do_optimize) # 这个函数帮助选择了下一个采样的点，并且GP优化也包含在里面了。
            ...
            # Estimate incumbent
            best_idx = np.argmin(self.y) # 目前已知的y中的最小值的index
            incumbent = self.X[best_idx] # 该index对应的X的值 叫做incumbent
            incumbent_value = self.y[best_idx] # 该index对应的y的值
